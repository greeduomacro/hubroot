    ...
    public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable, ISpawnable
    {
        ...
        public virtual void OnDeath( Container c )
        {
            ...
            else
            {
                ...
                //Body = this.StdRace.GhostBody( this );
        ...
        // XXX LOS BEGIN
        //		public virtual bool CanSee( object o, bool addLos )
        //		{
        //			if ( o is Item )
        //			{
        //				return CanSee( (Item) o, addLos );
        //			}
        //			else if ( o is Mobile )
        //			{
        //				return CanSee( (Mobile) o, addLos );
        //			}
        //			else
        //			{
        //				return true;
        ...
        //		{
        //            return CanSee( item, false );
        //        }
        //
        //		public virtual bool CanSee( Mobile mobile )
        //		{
        //            return CanSee( mobile, false );
        //        }
        // XXX LOS END
        ...
        public virtual bool CanSee( Item item )
        {
            ...
            if( m_Deleted || item.Deleted || m_Map == Map.Internal || item.Map == Map.Internal || m_Map != item.Map ) canSee = false;

            else if( item.Parent != null )
            {
                if( item.Parent is Item )
                {
                    if( !CanSee((Item)item.Parent) ) canSee = false;
                    else canSee =
                ...
                {
                    if( !CanSee((Mobile)item.Parent) ) canSee = false;
                    else canSee =
                        m_AccessLevel > AccessLevel.Counselor
                        ||
                        item.Visible
        ...
        public virtual bool CanSee( Mobile m )
        {
            // XXX LOS END
            // XXX LOS BEGIN
            //        if ( m_deleted || m.m_deleted || m_map == map.internal || m.m_map == map.internal ) return false;
            //        return this == m || (
            //            m.m_map == m_map &&
                    ...
                    if( ourState != null )
                    {
                        ...
                        foreach( object o in eable )
                        {
                            ...
                            else if( o != this && o is Mobile )
                            {
                                ...
                                {
                                    if( LOS.Config.GetInstance().SquelchNames > 0 ) m_LosRecent.Update(m, m);

                                    m.AddLos(this); // optimization
                                    if( m.m_NetState.IsSAClient )
                                    {
                                        m.m_NetState.Send(new MobileIncoming(m, this));
                                    ...
                                    if( IsDeadBondedPet ) m.m_NetState.Send(new BondedStatus(0, m_Serial, 1));
                                ...
                                )
                                {
                                    ...
                                    if( m.IsDeadBondedPet ) ourState.Send(new BondedStatus(0, m.m_Serial, 1));
        ...
        // XXX LOS BEGIN
        public Dictionary<Object, Object> LosCurrent { get { return m_LosCurrent; } }
