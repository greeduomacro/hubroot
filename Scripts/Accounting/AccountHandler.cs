...
namespace Server.Misc
{
	...
	{
		/// <summary>
		/// Server-side limitation on the number of characters an account can contain
		/// </summary>
		public static readonly int ServerLimit = 2;
		...
		private static TimeSpan DeleteDelay = TimeSpan.FromDays( 3.0 );
		...
		public static void Password_OnCommand( CommandEventArgs e )
		{
			...
			try
			{
				...
				else
				{
					...
					else if( PageQueue.AllowedToPage( from ) )
					{
						...
						PageQueue.AddNewPage( new PageEntry( from, String.Format( "[Automated: Change Password]<br>Desired password: {0}<br>Current IP address: {1}<br>Account IP address: {2}", pass, ipAddress, accessList[0] ), PageType.Other ) );
					}
		...
		private static void EventSink_DeleteRequest( DeleteRequestEventArgs e )
		{
			...
			else
			{
				...
				else if ( m.AccessLevel == AccessLevel.Player && Region.Find( m.LogoutLocation, m.LogoutMap ).GetRegion( typeof( Regions.Jail ) ) != null )	//Don't need to check current location, if netstate is null, they're logged out
				{
					state.Send( new DeleteResult( DeleteResultType.BadRequest ) );
					state.Send( new CharacterListUpdate( acct ) );
				}
