...
namespace Server.SkillHandlers
{
    class Meditation
    {
        ...
        public static bool CheckOkayHolding( Item item )
        {
            ...
            if( Core.SE && item is BaseWeapon && ((BaseWeapon)item).Attributes.SpellChanneling != 0 )
                return true;

            if( Core.SE && item is BaseArmor && ((BaseArmor)item).Attributes.SpellChanneling != 0 )
                return true;
        ...
        {
            EventSink.InvokeSkillUsed(new SkillUsedEventArgs(m, m.Skills[SkillName.Meditation]));
            ...
            else if( !Core.SE && m.Hits < (m.HitsMax / 10) ) // Less than 10% health
            {
                m.SendLocalizedMessage(501849); // The mind is strong but the body is weak.

                return TimeSpan.FromSeconds(5.0);
            }
            else if( m.Mana >= m.ManaMax && m.Hits >= m.HitsMax && m.Stam >= m.StamMax )
            {
                m.SendLocalizedMessage(501846); // You are at peace.

                return TimeSpan.FromSeconds(Core.AOS ? 10.0 : 5.0);
            }
            else if( Core.SE && Server.Misc.RegenRates.GetArmorOffset(m) > 0 )
            {
