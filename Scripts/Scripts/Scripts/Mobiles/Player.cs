...
namespace Server.Mobiles
{
    ...
    {        
        ...
        private BaseAlliance currentAlliance;

        public AllianceState allyState;

        public BaseAlliance CurrentAlliance
        {
        ...
        {
            if (this == from && this.Race == Race.Elemental && this.AbilityActive)
            {
                return;
            }
        ...
        public override void GetContextMenuEntries(Mobile from, List<ContextMenuEntry> list)
        {
            if (from == this)
            {
                ...
                if (!pm.Mounted || (Race == Race.Elemental && AbilityActive))
        ...
        public override void OnDeath(Container c)
        {
            ...
            SendGump(new RespawnGump(this, TimeSpan.FromMinutes((int)(KOCount/3)+1)));
        }
        ...
        {   
            ...
            {
                if (Race == Race.Liche && item is BaseStaff)
                    return base.OnEquip(item);

                if (Race == Race.Liche && !(item is BaseStaff))
                {
                    SendMessage("As a Liche you may only wield staff-type weapons.");
                    return false;
                }

                if (!( RaceBody == 0 || RaceBody == 400 || RaceBody == 401 ))
                {
                ...
                if (sum != null)
                {
                    if (sum.Puppeteer())
                    {
                        if(bmr != null && bmr.FollowerBonus()) 
                            return (base.FollowersMax + 5);
                        else 
                            return (base.FollowersMax + 2);
                    }
                }
                ...
                if(bmr != null)
                {
                    if(bmr.FollowerBonus())
                    {
                        if (sum != null && sum.Puppeteer())
                            ...
                            return (base.FollowersMax +3);
                    }
                }

                return base.FollowersMax;
            }
        ...
        public override void GetProperties( ObjectPropertyList list )
        {
            ...
            if( AccessLevel > AccessLevel.Player )
            {
                ...
                switch( AccessLevel )
                {
                    case AccessLevel.Counselor: color = "#5A5ABA"; break;
                    case AccessLevel.GameMaster: color = "#C10000"; break;
                    case AccessLevel.Seer: color = "#006A00"; break;
                    case AccessLevel.Administrator: color = "#010198"; break;
                }

                //list.Add(1060658, "{0}\t{1}", "Staff Team", String.Format("<BASEFONT COLOR={0}>{1}", color, AccessLevelLabels[(int)AccessLevel]));
            }
        }
        ...
        protected override bool OnMove( Direction d )
        {
            ...
            if (this is Player && this.Mounted && isRunning && this.Mount is BaseMount)
            {
                ...
                double mountCapacity = (double)(mount.Str * 3.75);
                stamLost = (double)(stamLost + (this.TotalWeight / mountCapacity));
                ...
                if (mount.Stam <= 0)
                {
                    this.SendMessage("Your steed is too fatigued to run!");
                    return false;
                }
            }
            ...
            if (this != null && this is Player && isRunning && this.Mounted == false )
            {
                ...
                double weightRatio = ((double)(this.TotalWeight) / (double)(this.MaxWeight));
                double htRatio = (40 / (Thirst + Hunger + 1));

                stamLost = ((stamLost + weightRatio) * htRatio) + 0.02;
            ...
            if (shoes == null && this is Player && !(legs is PlateLegs)) 
            {
                if(Utility.RandomDouble() == Utility.RandomDouble())
                {
