...
namespace Server.Mobiles
{
    ...
    public class BaseCreature : Mobile, IHonorTarget
    {
        ...
        public int KarmaLimit
        {
            get { return karmaLimit; }
            set { karmaLimit = value; }
        }
        ...
        private Item _spawner;
        private Point3D _spawnerHome;
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public Item SpawnerItem { get { return _spawner; } }

        [CommandProperty(AccessLevel.GameMaster)]
        public Point3D SpawnerHome { get { return _spawnerHome; } }
        ...
        */
        ...
        public virtual bool CanMoveOverObstacles
        {
            get
            {
                return Core.SE || this.Body.IsMonster;
            }
        }
        ...
        */
        ...
        public override void OnDamage( int amount, Mobile from, bool willKill )
        {
            ...
            EventDispatcher.InvokeCreatureDamaged(new CreatureDamagedEventArgs(this, amount, from, willKill));
        ...
        public override void Serialize( GenericWriter writer )
        {
            ...
            writer.Write((Item)_spawner);
            writer.Write((Point3D)_spawnerHome);
        ...
        public override void AddNameProperties( ObjectPropertyList list )
        {
            ...
            if( Controlled && Commandable && !Body.IsHuman )
            {
                if( Summoned )
        ...
        public override bool OnBeforeDeath()
        {
            ...
            {
                if( Map == Map.Backtrol && TreasureMap.LootChance >= Utility.RandomDouble() )
        ...
        public virtual void OnKilledBy( Mobile mob )
        {
            if( mob.Player )
            {
                EventDispatcher.InvokeCreatureKilled(new CreatureKilledEventArgs((Player)mob, this));
            }
        }

        public virtual bool AreSurroundingPlayers
        {
            get
            {
                List<NetState> clients = new List<NetState>();

                foreach (NetState client in this.GetClientsInRange(32))
                {
                    if (((Player)client.Mobile).ProduceCorpses)
                    {
                        clients.Add(client);
                    }
                }
        ...
        {
            if (this.CreatureAlignment == Alignment.None || this.CreatureAlignment == null)
            {
                if (Karma > -8000 && Karma < -2000)
                    this.CreatureAlignment = Alignment.Chaotic;

                if (Karma > 0 && Karma < 8000)
                    this.CreatureAlignment = Alignment.Lawful;

                if (Karma <= -8000)
                    this.CreatureAlignment = Alignment.Evil;

                if (Karma >= 8000)
                    this.CreatureAlignment = Alignment.Good;
            }

            if (this.Combatant == null && Utility.RandomDouble() < 0.33 && this.CreatureAlignment != Alignment.None
                && this.CreatureAlignment != Alignment.Neutral)
            {
                switch (alignment)
                {
                    case Alignment.Good:
                ...
                {
                    if (m != null && this.CanSee(m))
                    {
                        if (m is Player && ((Player)m).AccessLevel > AccessLevel.Player)
                            return;

                        if (CreatureCharge == Charge.Negative && m.Karma >= KarmaLimit)
                        {           
                            this.DoHarmful(m);
                        }

                        if (CreatureCharge == Charge.Positive && m.Karma <= KarmaLimit)
                        {
                            this.DoHarmful(m);
                        }
                        ...
                        if (m is BaseCreature)
                        {
                            ...
                            if (CreatureCharge == Charge.Negative && (bc.CreatureAlignment == Alignment.Lawful
                                || bc.CreatureAlignment == Alignment.Good))
                            {
                                this.DoHarmful(m);
                            }

                            if (CreatureCharge == Charge.Positive && (bc.CreatureAlignment == Alignment.Chaotic
                                || bc.CreatureAlignment == Alignment.Evil))
                            {
                                this.DoHarmful(m);
                            }
                        }
        ...
        public virtual bool PlayerRangeSensitive { get { return false; } }	//If they are following a waypoint, they'll continue to follow it even if players aren't around
        ...
        protected override void OnTick()
        {
            ...
            foreach( Mobile m in World.Mobiles.Values )
            {
                ...
                if( m is BaseCreature )
                {
                    ...
                    if( c.Body.IsHuman && c.ControlMaster != null )
                        continue;
                    else if( c.Blessed || c.IsBonded )
                        continue;
                    else if( c.Map == Map.Internal || c.IsStabled )
                        continue;
                    else if( c.IsDeadPet )
                    {
