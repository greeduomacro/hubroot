...
namespace Server.Guilds
{
    ...
    public class AllianceInfo
    {
        ...
        public void AllianceChat( Mobile from, int hue, string text )
        {
            ...
            foreach( KeyValuePair<Mobile, List<Guild>> kvp in Server.Commands.ListenToAlliance.Table )
            {
                for( int i = 0; i < m_Members.Count; i++ )
                {
                    if( kvp.Value.Contains(m_Members[i]) && !m_Members[i].IsMember(kvp.Key) && !kvp.Key.InRange(from, 8) )
                        kvp.Key.SendMessage(0x3B2, "<Alliance: {0}> [{1}]: {2}", this.Name, from.RawName, text);
    ...
    public class Guild : BaseGuild
    {
        ...
        public static readonly int RegistrationFee = 10;
        public static readonly int AbbrevLimit = 5;
        public static readonly int NameLimit = 30;
        ...
        public void AddMember( Mobile m )
        {
            if( !m_Members.Contains(m) )
            {
                ...
                ((Player)guild.Leader).CurrentAlliance.Add(m);
            }
        }
        ...
        public void GuildChat( Mobile from, int hue, string text )
        {
            ...
            foreach( KeyValuePair<Mobile, List<BaseGuild>> kvp in Server.Commands.ListenToGuild.Table )
            {
                if( kvp.Value.Contains(this) && !this.IsMember(kvp.Key) && !kvp.Key.InRange(from, 8) )
                    kvp.Key.SendMessage(0x3B2, "<Guild: {0}> [{1}]: {2}", this.Abbreviation, from.RawName, text);
            }
        }
