...
namespace Aberration.ContextMenus
{
                ...
                case Race.Liche:
                    {
                        ...
                        else
                        {
                            ...
                            pm.NextRaceAbility = TimeSpan.FromMinutes(3.0);
                        }
                        break;
                    }
                ...
                case Race.Elemental:
                    {
                        if (pm.AbilityActive)
                        {
                            ...
                            pm.BodyDamageBonus = 0;
                            pm.AbilityActive = false;
                            pm.PublicOverheadMessage(MessageType.Regular, pm.EmoteHue, false, String.Format("*{0} begins to solidify, taking the form of a human*", pm.Name));
                            pm.CanSwim = false;
                            ...
                            try
                            {
                                ...
                                mount.Rider = null;
                                ((EtherealSeaHorse)mount).Consume();
                            }
                        ...
                        else
                        {
                            ...
                            pm.RaceBody = 16;
                            pm.BodyDamageBonus = 12;
                            pm.AbilityActive = true;
                            pm.PublicOverheadMessage(MessageType.Regular, pm.EmoteHue, false, String.Format("*{0}'s form begins to turn to liquid*", pm.Name));
                            pm.CanSwim = true;
    ...
    public class ShapeshifterTarget : Target
    {
        ...
        protected override void OnTargetFinish(Mobile from)
        {
            if (hasChanged)
            {
                ((Player)from).NextRaceAbility = TimeSpan.FromMinutes(1.5);
                from.FixedParticles(14089, 0, 30, 0, 1155, 7, EffectLayer.CenterFeet);
            }
        }
    }
    ...
    public class OgreTarget : Target
    {
        ...
        protected override void OnTarget(Mobile from, object target)
        {
            if (target is Mobile && target != from)
            {
                ...
                ((Player)from).NextRaceAbility = TimeSpan.FromMinutes(1);
            }
