...
namespace Server.Spells
{
    public abstract class Spell : ISpell
    {
        ...
        {
            if( IsCasting && BlocksMovement && Caster.AccessLevel == AccessLevel.Player )
            {
        ...
        {
            return (Caster.AccessLevel == AccessLevel.Player && m_Info.AllowTown) || (Caster.AccessLevel > AccessLevel.Player);
        }

        public virtual bool ConsumeReagents()
        {
            if( m_Scroll != null || !m_Caster.Player || m_Caster.AccessLevel > AccessLevel.Player )
                return true;
        ...
        public virtual double GetDamageScalar( Mobile target )
        {
            ...
            if( !Core.ML )	//EvalInt stuff for AoS is handled elsewhere
            {
