...
namespace Server.SkillCapSelection {
	public class SkillCapSelectionGump : Gump {
		public const double MaxIndividualCap = 120.0;

		private Mobile _from;
		private SkillGroup _skillsGroup;

		public SkillCapSelectionGump( Mobile from )
			: this( from, null ) {
		}
							...
							if( sk != null ) {
								switch( mode ) {
									case "inc": {
                                        ...
                                        if( sk.Cap >= 200.0 ) 
                                            {
												_from.SendMessage( "That skill is at its maximum capacity. It cannot be raised any further" );
											}

                                        else if (((Player)_from).EoC < 15000)
                                        {
                                            _from.SendMessage("You need 15,000 Essence of Character to increase this skills cap.");
                                        } 

                                            else
                                            {
												sk.Cap += 5.0;
                                                ((Player)_from).EoC -= 15000;
											}
									...
									case "dec": {
											if( sk.Cap <= 100.0 ) {
												_from.SendMessage( "That skill is at its minimum capacity. It cannot be lowered further." );
											} 

                                            else 
                                            {
												sk.Cap -= 5.0;
                                                ((Player)_from).EoC += 5000;
                                                if (sk.Base > sk.Cap)
                                                    sk.Base = sk.Cap;
											}
