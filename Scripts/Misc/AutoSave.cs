...
namespace Server.Misc
{
	...
	{
		private static TimeSpan m_Delay = TimeSpan.FromHours( 0.5 );
		...
		public static void Save( bool permitBackgroundWrite )
		{
			...
			NetState.Instances.ForEach(
				delegate( NetState state )
				{
					if( state.Mobile != null )
						state.Mobile.SendGump( new Server.Gumps.SaveGump() );
				} );
			...
			NetState.Instances.ForEach(
				...
				{
					if( state.Mobile != null )
						state.Mobile.CloseGump( typeof( Server.Gumps.SaveGump ) );
				} );
		}
